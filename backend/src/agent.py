...
# Voice-Only Interactive Storytelling Agent
#
# Description:
#   This agent functions as a Dungeons & Dragons–style Game Master,
#   delivering a voice-driven narrative adventure set in a low-magic
#   coastal fantasy world (Brinmere). The story progresses entirely
#   through speech, with the player choosing actions to shape the path.
#
# Core Architecture:
#   - Built using LiveKit Voice Agents (STT, TTS, VAD, turn detection).
#   - Adventure logic controlled via scene dictionaries and tool functions.
#   - No external DB — story state stored per session in Userdata.
#   - Ideal starting point for extending into a full RPG engine.
#
# Key GM Tools:
#   start_adventure()    → initializes session + introduces opening scene
#   get_scene()          → returns current scene overview (always ends with "What do you do?")
#   player_action(...)   → interprets player speech, resolves choices, updates scene/journal/inventory
#   show_journal()       → displays remembered facts, choices, and collected items
#   restart_adventure()  → resets world and begins story anew
#
# Userdata Persisted State:
#   - current_scene       – where the player currently is
#   - history             – scene transitions + decisions
#   - inventory           – collected items (keys, artifacts, etc.)
#   - journal             – major clues, story notes, discoveries
#   - named_npcs          – future expansion for characters met
#   - choices_made        – helpful for analytics or flashbacks
#
# Notes for Future Expansion:
#   ✓ Add dice-roll mechanics (D20 outcomes, success tiers)
#   ✓ Add multiple universes (fantasy, sci-fi, cyberpunk)
#   ✓ Save/load JSON worldstate for long-form campaigns
#   ✓ Introduce NPC dialogue system + quests
#
# Modify + Expand Freely — this file is your adventure engine.
# ================================================================
