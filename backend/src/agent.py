"""
Day 10 – Voice Improv Battle (Murf AI Voice Agent Challenge)

A voice-first improv game host powered by LiveKit.  
This agent presents short creative scenarios, listens to player improv,
responds with varied feedback, tracks rounds, and finally summarizes the show.

Tools used by the host:
- start_show() — begin session + introduce rules
- next_scenario() — move to next improv round
- record_performance() — evaluate player performance + react
- summarize_show() — final recap of style/rounds
- stop_show() — graceful exit handling

High-energy, fast-flowing, built for real-time voice gameplay.
"""
