...
Features:
- SQLite-based order management (DB: order_db.sqlite)
- Pre-seeded Indian grocery catalog (Amul, Tata, Britannia, etc.)
- Cart Operations:
    • find_item (search catalog)
    • add_to_cart / remove_from_cart / update_cart_quantity / show_cart
- Recipe & Smart Ingredient Handling:
    • add_recipe (Chai, Maggi, Paneer Butter Masala, etc.)
    • ingredients_for (natural language dish → required items)
- Order Management:
    • place_order (stores in SQLite + starts live delivery simulation)
    • cancel_order (allowed if not delivered yet)
    • get_order_status / order_history (track + historic lookup)
- Background Simulation:
    • Order status auto-progresses every ~5 seconds

Purpose:
A fully voice-controlled grocery ordering agent integrating catalog lookup,
cart flow, recipe inference, order persistence, and real-time tracking.
"""
...
class FoodAgent(Agent):
    def __init__(self):
        super().__init__(
            instructions="""
            You are 'Robin', a helpful assistant for 'Dr Ortho Shop', an Indian grocery store.
...
async def entrypoint(ctx: JobContext):
    ...
    logger.info("\n" + "🇮🇳" * 12)
    logger.info("🚀 STARTING DR Ortho SHOP (Indian Context + Auto-Tracking)")
